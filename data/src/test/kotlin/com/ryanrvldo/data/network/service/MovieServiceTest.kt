...
internal class MovieServiceTest : BaseServiceTest() {
    ...
    override fun setUp() {
        super.setUp()
        service = NetworkModule.provideMovieService(fakeRetrofit)
    }
    ...
    inner class GetMovieByCategory {
        ...
        internal fun testGetPopularMoviesThenValidateRequest() {
            givenSuccessResultThenAssertCorrectRequest(
                HttpMethod.GET,
                "/movie/$POPULAR?page=1&api_key=${BuildConfig.TMDB_API_KEY}"
            ) {
    ...
    inner class GetMovieDetails {
        ...
        internal fun testGetMovieByIdThenValidateRequest() {
            givenSuccessResultThenAssertCorrectRequest(
                HttpMethod.GET,
                "/movie/$dummyId?append_to_response=${MovieService.DETAIL_APPEND_QUERY}&api_key=${BuildConfig.TMDB_API_KEY}"
            ) {
    ...
    inner class MovieDispatcher : QueueDispatcher() {
        override fun dispatch(request: RecordedRequest): MockResponse {
            ...
            return when {
                ...
                path.matches("/movie/+$POPULAR\\?page=1.*".toRegex()) -> enqueueResponse(
                    responseBody = POPULAR_MOVIES_PAGE_1
                )
                path.matches("/movie/+$POPULAR\\?page=2.*".toRegex()) -> enqueueResponse(
