...
suspend fun <T> withRetry(
    ...
    repeat(maxAttempts) { attempt ->
        val response: Result<T> = runCatching { block() }
        ...
        when {
            ...
            response.isFailure -> {
                ...
                if (attempt == maxAttempts - 1 || !shouldRetry(exception)) {
                    throw exception.asNetworkException()
                }
...
internal val HttpException.retryAfter: Long?
    get() {
...
internal fun defaultShouldRetry(throwable: Throwable) = when (throwable) {
